...
const LICENSE_CONFIG = {
  ...
  // License validation algorithm (simple checksum for offline validation)
...
function validateKeyFormat(key) {
  ...
  // Check pattern
  if (!LICENSE_CONFIG.LICENSE_PATTERN.test(key)) return false;

  // Simple checksum validation (last segment)
  const segments = key.split('-');
  const checksum = segments[4];
  const baseSegments = segments.slice(1, 4).join('');

  // Calculate expected checksum (simple algorithm for demo)
  const expectedChecksum = calculateChecksum(baseSegments);

  return checksum === expectedChecksum;
}

/**
 * Calculate checksum for license key segments
 * @param {string} data
 * @returns {string}
 */
function calculateChecksum(data) {
  ...
  // Convert to base36 and pad to 4 chars
  return sum.toString(36).toUpperCase().padStart(4, '0').slice(-4);
}

/**
 * Validate license key with API (online validation)
 * @param {string} key
...
function generateLicenseKey() {
  ...
  const baseSegments = segment1 + segment2 + segment3;
  const checksum = calculateChecksum(baseSegments);

  return `BLUR-${segment1}-${segment2}-${segment3}-${checksum}`;
}
